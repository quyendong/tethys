...
ajax_urls = [
    ...
    url(r'^(?P<job_id>[\d.@+-]+)/action/delete$', jobs_table_views.delete, name='delete_job'),
    url(r'^(?P<job_id>[\d.@+-]+)/action/resubmit$', jobs_table_views.resubmit, name='resubmit_job'),
    url(r'^(?P<job_id>[\d.@+-]+)/action/show-log', jobs_table_views.show_log, name='show_log'),
    url(r'^(?P<job_id>[\d.@+-]+)/log-content/(?P<key1>[\w+_-]+)(?:/(?P<key2>.*))?', jobs_table_views.get_log_content,
        name='log_content'),
    url(r'^(?P<job_id>[\d.@+-]+)/action/update-row', jobs_table_views.update_row, name='update_job_row'),
    url(r'^(?P<job_id>[\d.@+-]+)/action/update-workflow-nodes-row', jobs_table_views.update_workflow_nodes_row,
        name='update_workflow_nodes_row'),
    url(r'^(?P<job_id>[\d.@+-]+)/(?P<type>[\w-]+)/insert-bokeh-row', jobs_table_views.bokeh_row, name='bokeh_row'),
    url(r'^(?P<job_id>[\d.@+-]+)/action/(?P<action>[\w+-]+)', jobs_table_views.perform_action,
        name='custom_action'),
]
